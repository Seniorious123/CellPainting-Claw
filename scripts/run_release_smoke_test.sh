...
PYTHON_BIN="${PYTHON_BIN:-/root/miniconda3/envs/lyx_env/bin/python}"
...
PYTHONPATH=src "$PYTHON_BIN" -m cellpaint_pipeline show-config \
  --config "$CONFIG_PATH" \
  > "$OUTPUT_ROOT/show_config.json"

echo "[2/3] smoke-test"
PYTHONPATH=src "$PYTHON_BIN" -m cellpaint_pipeline smoke-test \
  --config "$CONFIG_PATH" \
...
if [ "$RUN_FAST_TESTS" = "1" ]; then
  ...
  PYTHONPATH=src "$PYTHON_BIN" -m cellpaint_pipeline.test_suites fast \
